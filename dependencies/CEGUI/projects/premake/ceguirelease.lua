...
do
    ...
    -- Cambio Marco Antonio Gomez: en vez de escribir
    -- directamente la configuracion en el fichero config.h,
    -- la metemos en una cadena, y sólo la mandamos a disco
    -- si el contenido del fichero es distinto. De esa forma
    -- la fecha no cambia, y se evitan recompilaciones
    -- innecesarias si se regenera la solución.
...
--    if not f then error("Unable to write config.h: "..err) end
    ...
    configContent = configContent .. "// config.h - generated by premake\n"
    configContent = configContent .. "// do not edit by hand, instead edit config.lua and regenerate\n"
    configContent = configContent .. "// it is important that you use premake recreate this file if changing xml parsers or codecs for the static builds\n"

    for k,v in pairs(config_h) do
        configContent = configContent .. "#define " .. k .. " " .. v .."\n"
    end
    ...
    if DEBUG_DLL_SUFFIX then
        configContent = configContent .. "#if defined(_DEBUG) || defined(DEBUG)\n"
        configContent = configContent .. "#   define CEGUI_HAS_BUILD_SUFFIX\n"
        configContent = configContent .. "#   define CEGUI_BUILD_SUFFIX \""..DEBUG_DLL_SUFFIX.."\"\n"
        configContent = configContent .."#endif\n"
    end

--    f:close()

    write_file("../../cegui/include/config.h", configContent)
end

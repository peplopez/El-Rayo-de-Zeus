...
//////////////////////////////
	...
	al OnStop().
	*/
	CLatentAction::LAStatus CLA_Walk::OnStart()
	{
		...
		message->setUShort( Logic::WALK );
		message->setBool(true);
		_entity->emitMessage(message);		
		
		//esta es la manera en la que propongo gestionar andar, que no lo contemplabamos hasta ahora puesto que nuestro heroe tan solo corre.
		if (_entity->hasComponent<CAvatarController>())
			_entity->getComponent<CAvatarController>()->setWander(true);
	...
	*/
	void CLA_Walk::OnStop()
	{
		//esta es la manera en la que propongo gestionar andar, que no lo contemplabamos hasta ahora puesto que nuestro heroe tan solo corre.
		if (_entity->hasComponent<CAvatarController>())
			_entity->getComponent<CAvatarController>()->setWander(false);
	}
	...
	*/
	CLatentAction::LAStatus CLA_Walk::OnRun() 
	{
		// TODO PRÁCTICA IA
	...
	de OnStop().
	*/
	CLatentAction::LAStatus CLA_Walk::OnAbort() 
	{
	...
	por ese mensaje.
	*/
	bool CLA_Walk::accept(const CMessage *message)
	{		
		return false;
	}
	/**
	...
	*/
	void CLA_Walk::process(CMessage *message)
	{
	}
	
	void CLA_Walk::tick(unsigned int msecs) 
	{
		CLatentAction::tick();
	}

	void CLA_Walk::sleepComponents(){}

	void CLA_Walk::awakeComponents(){}
} //namespace LOGIC